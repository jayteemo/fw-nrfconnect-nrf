...
BUILD_ASSERT(sizeof(CONFIG_REST_API_TOKEN) > 1, "API token must be specified");
...
retry:
	...
	err = nrf_cloud_rest_location_get(&rest_ctx, &loc_req, &location);
	if (err) {
		LOG_ERR("Locate API call failed, error: %d", err);
	} else {
		LOG_INF("Locate Response: %s", log_strdup(rest_ctx.response));
	}
...
pgps_start:
	...
	if (err) {
		...
		LOG_INF("P-GPS data host/path: %s/%s",
