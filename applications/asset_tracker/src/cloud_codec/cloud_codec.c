...
struct sensor_ch_cfg {
	double value[SENSOR_CH_CFG_ITEM_TYPE__END];
};

struct cloud_sensor_ch_cfg {
	enum cloud_channel ch;
	struct sensor_ch_cfg cfg;
};
...
static CMD_NEW_GROUP(
	group_set, CLOUD_CMD_GROUP_SET,
	CMD_ARRAY(
		CMD_NEW_RECIPIENT(
			environment, CLOUD_RCPT_ENVIRONMENT,
			CMD_ARRAY(
				CMD_NEW_CHAN(
					humidity, CLOUD_CHANNEL_HUMID,
					CMD_ARRAY(
						CMD_NEW_TYPE(enable,
							     CLOUD_CMD_ENABLE),
						CMD_NEW_TYPE(
				...
				CMD_NEW_CHAN(
					pressure, CLOUD_CHANNEL_AIR_PRESS,
					CMD_ARRAY(
						CMD_NEW_TYPE(enable,
							     CLOUD_CMD_ENABLE),
						CMD_NEW_TYPE(
						...
						CMD_NEW_TYPE(
							...
							CLOUD_CMD_THRESHOLD_LOW))),
				CMD_NEW_CHAN(
					temperature, CLOUD_CHANNEL_TEMP,
					CMD_ARRAY(
						CMD_NEW_TYPE(enable,
							     CLOUD_CMD_ENABLE),
						CMD_NEW_TYPE(
						...
						CMD_NEW_TYPE(
							...
							CLOUD_CMD_THRESHOLD_LOW))),
				CMD_NEW_CHAN(
					air_quality, CLOUD_CHANNEL_AIR_QUAL,
					CMD_ARRAY(
						CMD_NEW_TYPE(enable,
							     CLOUD_CMD_ENABLE),
						CMD_NEW_TYPE(
						...
						CMD_NEW_TYPE(
							...
							CLOUD_CMD_THRESHOLD_LOW))))),
		CMD_NEW_RECIPIENT(
			motion, CLOUD_RCPT_MOTION,
			CMD_ARRAY(
				CMD_NEW_CHAN(
					flip, CLOUD_CHANNEL_FLIP,
					CMD_ARRAY(
						CMD_NEW_TYPE(enable,
							     CLOUD_CMD_ENABLE),
						CMD_NEW_TYPE(
		...
		CMD_NEW_RECIPIENT(
			...
			CMD_ARRAY(
				CMD_NEW_CHAN(
					pin, CLOUD_CHANNEL_PIN,
					CMD_ARRAY(CMD_NEW_TYPE(enable,
							       CLOUD_CMD_ENABLE),
						  CMD_NEW_TYPE(pwm,
							       CLOUD_CMD_PWM))),
				CMD_NEW_CHAN(
					led, CLOUD_CHANNEL_RGB_LED,
					CMD_ARRAY(
						CMD_NEW_TYPE(red,
							     CLOUD_CMD_LED_RED),
						CMD_NEW_TYPE(
				...
				CMD_NEW_CHAN(
					...
					CMD_ARRAY(
						...
						CMD_NEW_TYPE(
							...
							CLOUD_CMD_PLAY_NOTE))))),
		CMD_NEW_RECIPIENT(
			light, CLOUD_RCPT_LIGHT,
			CMD_ARRAY(
				CMD_NEW_CHAN(
					red, CLOUD_CHANNEL_LIGHT_RED,
					CMD_ARRAY(
						CMD_NEW_TYPE(enable,
							     CLOUD_CMD_ENABLE),
						CMD_NEW_TYPE(
						...
						CMD_NEW_TYPE(
							...
							CLOUD_CMD_THRESHOLD_LOW))),
				CMD_NEW_CHAN(
					green, CLOUD_CHANNEL_LIGHT_GREEN,
					CMD_ARRAY(
						CMD_NEW_TYPE(enable,
							     CLOUD_CMD_ENABLE),
						CMD_NEW_TYPE(
	...
	CMD_ARRAY(
		CMD_NEW_RECIPIENT(
			...
			CMD_ARRAY(
				CMD_NEW_CHAN(
					humidity, CLOUD_CHANNEL_HUMID,
					CMD_ARRAY(CMD_NEW_TYPE(read,
							       CLOUD_CMD_READ),
						  CMD_NEW_TYPE(read_all,
							       CLOUD_CMD_READ))),
				CMD_NEW_CHAN(
					pressure, CLOUD_CHANNEL_AIR_PRESS,
					CMD_ARRAY(CMD_NEW_TYPE(read,
				...
				CMD_NEW_CHAN(
					...
					CMD_ARRAY(CMD_NEW_TYPE(read,
						  ...
						  CMD_NEW_TYPE(
							  ...
							  CLOUD_CMD_READ))))),
		CMD_NEW_RECIPIENT(
			modem_info, CLOUD_RCPT_MODEM_INFO,
			CMD_ARRAY(
				CMD_NEW_CHAN(
					device, CLOUD_CHANNEL_DEVICE_INFO,
					CMD_ARRAY(CMD_NEW_TYPE(read,
							       CLOUD_CMD_READ),
						  CMD_NEW_TYPE(read_all,
							       CLOUD_CMD_READ),
						  CMD_NEW_TYPE(
							  ...
							  CLOUD_CMD_READ_NEW))),
				CMD_NEW_CHAN(
					rsrp, CLOUD_CHANNEL_LTE_LINK_RSRP,
					CMD_ARRAY(CMD_NEW_TYPE(read,
							       CLOUD_CMD_READ),
						  CMD_NEW_TYPE(
							  read_all,
...
struct cloud_sensor_ch_cfg sensor_cfg[] = {
	{ .ch = CLOUD_CHANNEL_TEMP,
	  .cfg = { .value = { [SENSOR_CH_CFG_ITEM_TYPE_SEND_ENABLE] = true } } },
	{ .ch = CLOUD_CHANNEL_HUMID,
	  .cfg = { .value = { [SENSOR_CH_CFG_ITEM_TYPE_SEND_ENABLE] = true } } },
	{ .ch = CLOUD_CHANNEL_AIR_PRESS,
	  .cfg = { .value = { [SENSOR_CH_CFG_ITEM_TYPE_SEND_ENABLE] = true } } },
	{ .ch = CLOUD_CHANNEL_AIR_QUAL,
	  .cfg = { .value = { [SENSOR_CH_CFG_ITEM_TYPE_SEND_ENABLE] = true } } },
	{ .ch = CLOUD_CHANNEL_LIGHT_RED,
	  .cfg = { .value = { [SENSOR_CH_CFG_ITEM_TYPE_SEND_ENABLE] = true } } },
	{ .ch = CLOUD_CHANNEL_LIGHT_GREEN,
	  .cfg = { .value = { [SENSOR_CH_CFG_ITEM_TYPE_SEND_ENABLE] = true } } },
	{ .ch = CLOUD_CHANNEL_LIGHT_BLUE,
	  .cfg = { .value = { [SENSOR_CH_CFG_ITEM_TYPE_SEND_ENABLE] = true } } },
	{ .ch = CLOUD_CHANNEL_LIGHT_IR,
	  .cfg = { .value = { [SENSOR_CH_CFG_ITEM_TYPE_SEND_ENABLE] = true } } },
};
...
static int
cloud_cmd_handle_sensor_set_ch_cfg(struct cloud_command const *const cmd);
	...
	for (size_t i = 0; i < cmd_group->num_children; i++) {
		...
		for (size_t j = 0; j < rcpt.num_children; j++) {
			...
			for (size_t k = 0; k < chan.num_children; k++) {
...
#if !IS_ENABLED(CONFIG_NEWLIB_LIBC_FLOAT_SCANF)
					int int_val = 0;
					if (sscanf(type_obj->valuestring, "%d",
						   &int_val) != 1)
#else
					if (sscanf(type_obj->valuestring, "%lf",
						   &cmd_parsed.value) != 1)
#endif
					{
						continue;
				...
				if ((cmd_group == &group_set) &&
				    (cloud_cmd_handle_sensor_set_ch_cfg(
					     &cmd_parsed) == 0)) {
					/* no need to pass to cb if cmd was successfully handed,  */
					continue;
				}
				  ...
				  struct cloud_msg *output)
{
	...
	len = snprintf(buf, sizeof(buf), "%.1f", sensor_data->value);
...
static int sensor_ch_cfg_set_item(struct sensor_ch_cfg *const cfg,
				  const enum sensor_ch_cfg_item_type type,
				  const double value)
{
	if ((type < SENSOR_CH_CFG_ITEM_TYPE__BEGIN) ||
	    (type >= SENSOR_CH_CFG_ITEM_TYPE__END) || (cfg == NULL)) {
		return -EINVAL;
	}

	printk("set cfg type %d to %d\n", type, (int)value);
	cfg->value[type] = value;

	return 0;
}

static bool sensor_ch_cfg_is_send_allowed(const struct sensor_ch_cfg *const cfg,
					  const double sensor_value)
{
	if ((cfg == NULL) ||
	    (!cfg->value[SENSOR_CH_CFG_ITEM_TYPE_SEND_ENABLE])) {
		return false;
	}

	if (((cfg->value[SENSOR_CH_CFG_ITEM_TYPE_THRESH_LOW_ENABLE]) &&
	     (sensor_value <
	      cfg->value[SENSOR_CH_CFG_ITEM_TYPE_THRESH_LOW_VALUE]))) {
		return true;
	}

	if (((cfg->value[SENSOR_CH_CFG_ITEM_TYPE_THRESH_HIGH_ENABLE]) &&
	     (sensor_value >
	      cfg->value[SENSOR_CH_CFG_ITEM_TYPE_THRESH_HIGH_VALUE]))) {
		return true;
	}

	return (!cfg->value[SENSOR_CH_CFG_ITEM_TYPE_THRESH_LOW_ENABLE] &&
		!cfg->value[SENSOR_CH_CFG_ITEM_TYPE_THRESH_HIGH_ENABLE]);
}
			  ...
			  const double value)
{
	for (int i = 0; i < ARRAY_SIZE(sensor_cfg); ++i) {
		if (sensor_cfg[i].ch == channel) {
			return sensor_ch_cfg_set_item(&sensor_cfg[i].cfg, type,
						      value);
		}
	}
...
bool cloud_is_send_allowed(const enum cloud_channel channel, const double value)
{
	for (int i = 0; i < ARRAY_SIZE(sensor_cfg); ++i) {
		if (sensor_cfg[i].ch == channel) {
			return sensor_ch_cfg_is_send_allowed(&sensor_cfg[i].cfg,
							     value);
		}
	}
...
int cloud_cmd_handle_sensor_set_ch_cfg(struct cloud_command const *const cmd)
{
	int err = -ENOTSUP;
	...
	case CLOUD_CMD_ENABLE:
		err = cloud_set_ch_cfg_item(
			cmd->channel, SENSOR_CH_CFG_ITEM_TYPE_SEND_ENABLE,
