	...
	if (evt->type == CLOUD_EVT_CONNECTING) {
		...
		case CLOUD_DISCONNECT_ERR_INVALID_REQUEST:
			if ((atomic_get(&cloud_association) ==
				...
				connect_to_cloud(CONFIG_CLOUD_CONNECT_RETRY_DELAY);
			}
			break;
...
/* Usually the modem configuration is done during boot,
 * but when LWM2M carrier library is enabled, BSD library
 * is not enabled before now, leaving the configuration
 * to the application.
 */
	...
	/* modem andset up the LTE connection. */
