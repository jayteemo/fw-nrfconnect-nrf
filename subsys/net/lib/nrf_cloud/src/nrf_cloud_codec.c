...
static int cbor_decode(struct nrf_cloud_obj *const obj, const struct nrf_cloud_data *const input)
{
	/* CBOR decoding not yet supported */
	return  -ENOTSUP;
}
	...
	const struct nrf_cloud_data *const input)
{
	...
	if ((obj->type == NRF_CLOUD_OBJ_TYPE_CBOR) || !known_type) {
		ret = cbor_decode(obj, input);
		if ((ret == 0) || known_type) {
			return ret;
		}
	}
