	...
	size_t inf_cnt)
{
	...
	for (size_t i = 0; i < inf_cnt; ++i) {
		...
		if (lte->ncells_count) {
			nmr_array = cJSON_AddArrayToObject(lte_obj, NRF_CLOUD_CELL_POS_JSON_KEY_NBORS);
			if (!nmr_array) {
				goto cleanup;
			}
		}
		...
		for (uint8_t j = 0; nmr_array && (j < lte->ncells_count); ++j) {
			...
			/* required items */
			if (json_add_num(ncell_obj, NRF_CLOUD_CELL_POS_JSON_KEY_EARFCN, ncell->earfcn) ||
			    json_add_num(ncell_obj, NRF_CLOUD_CELL_POS_JSON_KEY_PCI, ncell->phys_cell_id)) {
				goto cleanup;
			}

			/* optional */
			if (json_add_num(ncell_obj, NRF_CLOUD_CELL_POS_JSON_KEY_RSRP, RSRP_ADJ(ncell->rsrp)) ||
			    json_add_num(ncell_obj, NRF_CLOUD_CELL_POS_JSON_KEY_RSRQ, ncell->rsrq)) {
				goto cleanup;
			}
		}
