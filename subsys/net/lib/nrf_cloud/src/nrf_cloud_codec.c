...
#define TOPIC_VAL_RCV_WILDCARD	"/+" TOPIC_VAL_RCV
			   ...
			   struct nrf_cloud_data *output)
{
	...
	case STATE_UA_PIN_COMPLETE: {
		...
		if (update_desired_topic) {
			...
			cJSON *des_pair_obj = cJSON_AddObjectToObjectCS(des_obj, JSON_KEY_PAIRING);
			cJSON *des_topic_obj = cJSON_AddObjectToObjectCS(des_pair_obj, JSON_KEY_TOPICS);

			ret += json_add_str_cs(des_topic_obj, JSON_KEY_CLOUD_TO_DEVICE, rx_endp.ptr);
		}
...
TODO: remove below... temp for testing until BE is updated */
	...
	if (strstr(topic, TOPIC_VAL_AGPS)) {
		return NRF_CLOUD_RCV_TOPIC_AGPS;
	} else if (strstr(topic, TOPIC_VAL_PGPS)) {
		return NRF_CLOUD_RCV_TOPIC_PGPS;
	} else if (strstr(topic, TOPIC_VAL_CELL_POS)) {
		return NRF_CLOUD_RCV_TOPIC_CELL_POS;
	} else if (strstr(topic, TOPIC_VAL_C2D)) {
		return NRF_CLOUD_RCV_TOPIC_GENERAL;
	} else {
		return NRF_CLOUD_RCV_TOPIC_UNHANDLED;
	}
