...
#define API_GET_SINGLECELL_TEMPLATE	API_VER "/location/single-cell?" DEV_ID \
					"&mcc=%u&mnc=%u&tac=%u&eci=%u&format=json"
/* TODO: update to use actual API format when available */
#define API_GET_MULTICELL_TEMPLATE	API_VER "/location/multi-cell?" DEV_ID \
					"&mcc=%u&mnc=%u&tac=%u&eci=%u&format=json"
...
#define PGPS_REQ_PREDICT_CNT		"&predictionCount=%u"
#define PGPS_REQ_PREDICT_INT_MIN	"&predictionIntervalMinutes=%u"
#define PGPS_REQ_START_GPS_DAY		"&startGpsDay=%u"
#define PGPS_REQ_START_GPS_TOD_S	"&startGpsTimeOfDaySeconds=%u"
...
int tls_setup(int fd, const char *const tls_hostname, const sec_tag_t sec_tag)
{
	int err;
	int verify;
...
static int do_cell_based_loc_call(struct nrf_cloud_rest_context *const rest_ctx,
	struct http_request *const http_req,
	...
	const enum cell_based_location_type type)
{
	...
	/* Format auth header */
	int ret = generate_auth_header(rest_ctx->auth, &auth_hdr);
...
int nrf_cloud_rest_scell_get(struct nrf_cloud_rest_context *const rest_ctx,
	struct nrf_cloud_rest_scell_request const *const request,
	struct nrf_cloud_cell_pos_result *const result)
{
	__ASSERT_NO_MSG(rest_ctx != NULL);
	...
	ret = snprintk(url, url_sz, API_GET_SINGLECELL_TEMPLATE,
		       request->device_id, request->net_info.mcc,
		       request->net_info.mnc, request->net_info.area_code,
		       request->net_info.cell_id);
	if (ret < 0 || ret >= url_sz) {
		LOG_ERR("Could not format URL");
		ret = -ETXTBSY;
		goto clean_up;
	...
	ret = do_cell_based_loc_call(rest_ctx, &http_req, result,
...
int nrf_cloud_rest_mcell_get(struct nrf_cloud_rest_context *const rest_ctx,
	struct nrf_cloud_rest_mcell_request const *const request,
	struct nrf_cloud_cell_pos_result *const result)
{
	__ASSERT_NO_MSG(rest_ctx != NULL);
	__ASSERT_NO_MSG(request != NULL);
	...
	/* TODO: update to use actual API format when available */
	/* Format API URL with device ID and cell data */
	url_sz = sizeof(API_GET_MULTICELL_TEMPLATE) +
		 strlen(request->device_id) +
		 NET_INFO_PRINT_SZ;
	url = k_calloc(url_sz, 1);
	if (!url) {
	...
	ret = snprintk(url, url_sz, API_GET_MULTICELL_TEMPLATE,
		       request->device_id, request->net_info.mcc,
		       request->net_info.mnc, request->net_info.area_code,
		       request->net_info.cell_id);
	if (ret < 0 || ret >= url_sz) {
