...
static int publish_and_free(struct nrf_cloud_obj *const pub_data,
...
static int send_job_update(struct nrf_cloud_fota_job *const job)
{
	...
	ret = publish_and_free(&update_obj, &topic_updt, &param);
...
int nrf_cloud_fota_ble_update_check(const bt_addr_t *const ble_id)
{
	...
	return publish_and_free(&check_obj, &topic_ble_req, &param);
}
				  ...
				  const enum nrf_cloud_fota_status status)
{
	...
	return publish_and_free(&job_obj, &topic_ble_updt, &param);
}
#endif
