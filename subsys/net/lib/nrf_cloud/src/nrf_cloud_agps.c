...
#define AGPS_JSON_CELL_LOC_KEY_ALT	"alt"
...
enum cell_based_location_state {
	CELL_LOC_STATE_NOT_INITD,
	CELL_LOC_STATE_INITD,
	CELL_LOC_STATE_LOC_RCVD,
};
				    ...
				    const enum cell_based_location_type type)
{
	...
	alt = cJSON_GetObjectItem(cell_loc_obj,
				  AGPS_JSON_CELL_LOC_KEY_ALT);
	...
	/* TODO: determine if alt. is a valid field for cell-based location  */
	if (cJSON_IsNumber(alt)) {
		cell_based_loc.alt = alt->valueint;
	}
	cell_based_loc.type = type;

	LOG_DBG("Cell location: (%lf, %lf), alt: %d, unc: %d, type: %d",
		cell_based_loc.lat, cell_based_loc.lon, cell_based_loc.alt,
